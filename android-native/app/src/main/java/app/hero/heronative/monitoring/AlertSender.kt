...
object AlertSender {
    suspend fun sendFallAlert(context: Context, session: UserSession): Result<Unit> {
        val gps = LocationProvider(context).getCurrentLocation()
        val location = HeroLocationDefaults.resolve(session.phone, gps)
            ?: return Result.failure(IllegalStateException("위치를 확인할 수 없습니다"))
        return runCatching {
